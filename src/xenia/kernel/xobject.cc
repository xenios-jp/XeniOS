...
DEFINE_bool(
    ...
    "CPU");
DEFINE_int32(wait_backoff_spin_polls, 64,
             "wait_timeout_backoff: consecutive immediate timeouts to keep "
             "cheap-yielding "
             "before parking.",
             "CPU");
...
DEFINE_int32(
    ...
    "latency.",
    "CPU");
DEFINE_int32(wait_backoff_reset_us, 1000,
             "wait_timeout_backoff: gap since the last immediate timeout that "
             "resets the "
...
// yields for short waits; after a sustained tight spin, park briefly so the
// core can idle. The inter-miss gap self-detects a tight loop, so a real
// blocking wait that merely times out keeps the cheap yield. Gated off by
// default.
